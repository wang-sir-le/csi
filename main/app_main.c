...
static void wifi_init()
{
    ...
    /*
     * Keep receiver's hardware MAC unchanged. CONFIG_CSI_SEND_MAC is the
     * fixed source MAC used by csi_send and is only used for CSI filtering.
     */
}
...
static void wifi_csi_rx_cb(void *ctx, wifi_csi_info_t *info)
{
    ...
    uint32_t rx_id = s_count;
#if CONFIG_IDF_TARGET_ESP32C5 || CONFIG_IDF_TARGET_ESP32C6 || CONFIG_IDF_TARGET_ESP32C61
    if (!s_count) {
        ESP_LOGI(TAG, "================ CSI RECV ================");
        ets_printf("type,seq,mac,rssi,rate,noise_floor,fft_gain,agc_gain,channel,local_timestamp,sig_len,rx_state,len,first_word,data,amplitude,phase\n");
    }
    ...
    if (!s_count) {
        ESP_LOGI(TAG, "================ CSI RECV ================");
        ets_printf("type,id,mac,rssi,rate,sig_mode,mcs,bandwidth,smoothing,not_sounding,aggregation,stbc,fec_coding,sgi,noise_floor,ampdu_cnt,channel,secondary_channel,local_timestamp,ant,sig_len,rx_state,len,first_word,data,amplitude,phase\n");
    }
    ...
    for (int i = 0; i < num_subcarriers; i++) {
        float real = compensate_gain * (float)info->buf[2*i + 1]; // 实部
        float imag = compensate_gain * (float)info->buf[2*i];     // 虚部
        float amplitude = sqrtf(real*real + imag*imag);
        if (i > 0) ets_printf(",");
        ets_printf("%.2f", amplitude);
    }
    ets_printf("]\"");
    ets_printf(",\"[");
    for (int i = 0; i < num_subcarriers; i++) {
        float real = compensate_gain * (float)info->buf[2*i + 1];
        float imag = compensate_gain * (float)info->buf[2*i];
